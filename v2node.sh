...
http_download() {
    ...
    if command -v curl &> /dev/null; then
        ...
        echo -e "${red}错误：系统中没有 curl 或 wget，请先安装其中之一${plain}"
        exit 1
    fi
}
...
install() {
    if command -v curl &> /dev/null; then
        ...
        bash <(wget -qO- https://raw.githubusercontent.com/wyx2685/v2node/master/script/install.sh)
    else
        echo -e "${red}错误：系统中没有 curl 或 wget${plain}"
        exit 1
    fi
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
...
update() {
    ...
    if command -v curl &> /dev/null; then
        ...
        bash <(wget -qO- https://raw.githubusercontent.com/wyx2685/v2node/master/script/install.sh) $version
    else
        echo -e "${red}错误：系统中没有 curl 或 wget${plain}"
        exit 1
    fi
...
update_shell() {
    if command -v wget &> /dev/null; then
        ...
        curl -sL -o /usr/bin/v2node https://raw.githubusercontent.com/wyx2685/v2node/master/script/v2node.sh
    else
        echo -e "${red}错误：系统中没有 curl 或 wget${plain}"
        before_show_menu
        return
