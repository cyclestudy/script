...
http_download_pipe() {
    ...
    if command -v curl &> /dev/null; then
        ...
        echo -e "${red}错误：系统中没有 curl 或 wget，请先安装其中之一${plain}" >&2
        exit 1
    fi
}
...
EOF
    ...
    http_download "https://raw.githubusercontent.com/wyx2685/v2node/main/script/v2node.sh" "/usr/bin/v2node"
